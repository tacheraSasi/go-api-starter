...
func main() {
	...
	notificationService := services.NewNotificationService(asynqClient)
	log.Printf("Notification service ready (redis=%s)", cfg.RedisURL)
	...
	// notificationService is available for any handler that needs to
	// enqueue async SMS / email tasks, e.g.:
	//   notificationService.SendSMSAsync("+255...", "Hello")
	//   notificationService.SendEmailAsync("u@example.com", "Subject", "Body")
	...
	monHandler := asynqmon.New(asynqmon.Options{
		RootPath:     "/monitoring",
		RedisConnOpt: redisOpt,
	})
	defer monHandler.Close()

	monitoring := r.Group("/monitoring")
	monitoring.Use(gin.BasicAuth(gin.Accounts{
		cfg.MonitoringUser: cfg.MonitoringPassword,
	}))
	monitoring.Any("/*path", gin.WrapH(monHandler))
	...
	go func() {
		log.Printf("Core server starting on :%s (monitoring at /monitoring)", cfg.ServerPort)
	...
	log.Println("Core server exited gracefully")
}
